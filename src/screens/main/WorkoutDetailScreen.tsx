/**
 * WorkoutDetailScreen — Fixed
 *
 * Bugs fixed:
 * 1. Unused `saving` state removed
 * 2. useFocusEffect se fresh workout data fetch on screen focus —
 *    agar ActiveWorkout se wapas aao toh completed status update ho jata hai
 * 3. workout.completed check ab refreshed data pe hota hai
 */
...
export default function WorkoutDetailScreen({ route, navigation }: any) {
  ...
  // Start with passed workout, refresh on focus to catch completion state changes
  ...
  // Refresh workout data every time screen is focused
  // (so "Completed" badge shows after returning from ActiveWorkoutScreen)
  useFocusEffect(
    useCallback(() => {
      let active = true;
      const refresh = async () => {
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }} edges={['top']}>
      ...
      >
        ...
        <TouchableOpacity
          ...
          style={[styles.backBtn, { paddingTop: insets.top + 16 }]}
        >
        ...
        >
          ...
          <View style={[styles.typeIconWrap, { backgroundColor: typeColor + '20' }]}>
            <AppIcon
              name={TYPE_ICONS[workout.type] ?? 'fitness'}
              size={40}
              color={typeColor}
            />
          </View>
          ...
          {workout.completed && (
            <View
              style={[
                styles.completedTag,
                { backgroundColor: colors.success + '22', borderColor: colors.success + '50' },
              ]}
            >
          ...
          <View style={[styles.statsRow, { borderTopColor: colors.divider }]}>
            {[
              ...
              { icon: 'fire', val: `${workout.calories || 0}`, lbl: 'Est. Kcal' },
              { icon: 'dumbbell', val: `${workout.exercises.length}`, lbl: 'Exercises' },
              {
                icon: 'calendar',
                val: dayjs(workout.scheduledDate).format('MMM D'),
                lbl: 'Scheduled',
              },
            ].map((s) => (
        ...
        {/* Start Workout button — only shown if not yet completed */}
        {!workout.completed && (
          <TouchableOpacity
        ...
        >
          ...
          {workout.exercises.length === 0 ? (
            <Text style={[styles.noExText, { color: colors.textSecondary }]}>
              No exercises added.
            </Text>
          ) : (
